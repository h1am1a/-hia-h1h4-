...
def INSTAGRAM():
    ...
    for data in checkurl1_2['data'] :
        ...
        print(f'•[</>]➭ [{i}] => Tên Tài Khoản : {usernametk} => Status : {STATUS[-1]}')      
        i += 1
    print('_________________________________________________________')    
    choose = int(input('[</>]   Nhập Tài Khoản : '))
    ...
    if choose >=1 or choose <= len(user_INS) :
        ...
        if checkfile2 == False:
            banner()
            cookieX = input('[</>] Nhập Cookie Instagram: ')
        ...
        os.system('cls' if os.name== 'nt' else 'clear')
        banner()
        choose = int(input('[</>] Nhập Số Lượng Job : '))
        headerINS = {
                'accept': '*/*',
                ...
                if nos['status'] == 200:
                    ...
                    if job_type == 'follow':
                        ...
                        if '"status":"ok"' in response:
                            ...
                            if response.get('success') == True:
                                ...
                                if checkskipjob['status'] == 200:
                                    print(str(checkskipjob['message']))

                        elif '"status":"fail"' in response and '"spam":true' in response:
                            print("Tài khoản này bị nhã Follow")
                        ...
                        if '"status":"ok"' in response:
                            ...
                            print("Tài khoản này bị chặn like")
                        elif '"status":"fail"' in response and '"require_login":true' in response:
                            print('Cookie die rồi! Tôi rất tiếc')
                            os.remove(f'COOKIEINS{account_id}.txt')
                            return 0
...
def LIST():
    banner()
    print("[</>] Nhập 1 để vào Tool Instagram")
os.system('cls' if os.name== 'nt' else 'clear')
banner()
checkfile = os.path.isfile('user.txt')
if checkfile == False:
    AUTHUR = input('[</>] NHẬP Authorization Golike : ')
...
if checkurl1['status']== 200 :
        ...
        print(Fore.WHITE+'[</>] Tài Khoản : '+Fore.WHITE+username)
        print(Fore.WHITE+'[</>] Tổng Tiền : '+Fore.WHITE+str(coin))
        print('_________________________________________________________')
        LIST()
        print('[</>] Nhập 2 Để Xóa Authorization Hiện Tại')
        choose = int(input(Fore.WHITE+'Nhập Lựa Chọn : '))
        if choose == 1:
            os.system('cls' if os.name== 'nt' else 'clear')
            banner()
            ip = requests.get('https://api.ipify.org?format=json').json()
            print('Danh Sách Tài Khoản'+ 'Ip : '+str(ip['ip']))
            print('_________________________________________________________\n')
