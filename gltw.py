...
def TWITTER():
    ...
    for data in checkurl1_2['data'] :
        ...
        print(f'● [</>] ➩ Nhập Số | {i} | => Tài Khoản: {usernametk}')
        i=i+1
    print('══════════════════════════════════════════════════════════════')
    ...
    if choose >=1 or choose <= len(user_twitter1) :
        ...
        checkfile = os.path.isfile('AuthTwitter'+str(account_id)+'.txt')
        if checkfile == False:
            banner()
            AUTHURX = input('● [</>] ➩ Auth Twitter: ')
            createfile = open('AuthTwitter'+str(account_id)+'.txt','w')
            createfile.write(AUTHURX)
            createfile.close()
            readfile = open('AuthTwitter'+str(account_id)+'.txt','r')
            ...
            AUTHURX = readfile.read()
            readfile.close()
        checkfile2 = os.path.isfile('CookieTwitter'+str(account_id)+'.txt')
        if checkfile2 == False:
            banner()
            cookieX = input(Colorate.Diagonal(Colors.green_to_cyan, '● [</>] ➩ Cookie Twitter: '))
            createfile = open('CookieTwitter'+str(account_id)+'.txt','w')
            createfile.write(cookieX)
            createfile.close()
            readfile = open('CookieTwitter'+str(account_id)+'.txt','r')
        ...
        os.system('cls' if os.name== 'nt' else 'clear')
        banner()
        choose = int(input(f'● [</>] ➩ Nhập Số Lượng Job: '))
                    ...
                    if nos['status'] ==200:
                        ...
                        if type=='like':
                            ...
                            if 'data' or 'has already favorited tweet' in str(node):
                                ...
                                print("Cookie die rồi! Vui lòng đổi cookie mới!")
                                os.remove('CookieTwitter'+str(account_id)+'.txt')
                                return 0
                        elif type == 'follow':
...
if checkurl1['status']== 200 :
        ...
        print('● [</>] ➩ Số Dư: '+str(coin))
        print('══════════════════════════════════════════════════════════════')
        print("● [</>] ➩ Nhập Số | 1 | Để Vào Tool Twitter")
